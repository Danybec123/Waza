var firebaseConfig = {
  ...
  function tenermensajes(){
    ...
    i.forEach(function(r){
      ...
      if (keya!="purpose") {
        ...
        h2="<h2>"+nombre+"</h2>"
        h3="<h3>"+mesaje+"</h3>"
        boton="<button onclick='onclickea(this.id)' id="+keya+" value="+likes+">Likes"+likes+"</button>"  
  ...
  function onclickea(l){
     var variable1 =l;
     var valor=document.getElementById(variable1).value
     var actualizar=Number (valor)+1
     firebase.database().ref(traer2).child(variable1).update({
      likes:actualizar
     })
  }
  tenermensajes()